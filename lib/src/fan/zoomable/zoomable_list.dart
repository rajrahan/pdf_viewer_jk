...
class _ZoomableListState extends State<ZoomableList> with TickerProviderStateMixin {
  ...
  void _handleReset() {
    _zoomAnimation = Tween<double>(begin: 1.0, end: _zoom).animate(CurvedAnimation(parent: _controller, curve: Curves.easeInOut))..addListener(() => setState(() => _zoom = _zoomAnimation.value));
    _panOffsetAnimation = Tween<Offset>(begin: Offset(0.0, _panOffset.dy), end: _panOffset).animate(CurvedAnimation(parent: _controller, curve: Curves.easeInOut))..addListener(() => setState(() => _panOffset = _panOffsetAnimation.value));
    if (_zoom < 0)
